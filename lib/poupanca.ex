defmodule Poupanca do
  ...
  def investir(de, para, valor_investido) do
    ...
    cond do
      ...
      valida_saldo(de.saldo, valor_investido) ->
  ...
  def retirar(conta_poupanca, valor_retirado) do
    ...
    cond do
      ...
      valida_saldo(conta_poupanca.saldo, valor_retirado) ->
        {:error, "Saldo insuficiente!"}
  ...
  def lucros(data) do
    # Se a data atual for igual a uma data especifica faça
    if data == ~D[2022-02-11] do
      contas = Poupanca.deletar([busca_contas()])
      # Atualizando todos os saldos de todas as contas usando o Enum.map
      contas =
        Enum.map(contas, fn x ->
          %Poupanca{x | saldo: Float.round(x.saldo + x.saldo * 0.0076, 2)}
        end)
      ...
      File.write(@contas, :erlang.term_to_binary(contas))
      "Lucros adicionados nas contas com sucesso!"
      # Caso contrário
    else
      "Nao esta no dia para lucros!"
    end
  end
